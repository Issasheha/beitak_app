...
//   ServiceDetailsViewModel({Dio? dio, AuthLocalDataSource? local})
...
//       throw ServerException(message: 'Invalid response format', statusCode: code);
//     }
//     return body;
//   }
...
//       state.error = 'Access token required';
//       return;
//     }

//     try {
//       final res = await _dio.get('/bookings/$bookingId', options: _opts(token));
...
//         state.data = data['booking'] is Map<String, dynamic>
//             ? data['booking'] as Map<String, dynamic>
//             : data;
//       } else {
//         throw const ServerException(message: 'Invalid booking details format');
//       }
//     } on DioException catch (e) {
//       if (e.response?.data is Map<String, dynamic>) {
//         state.error = (e.response!.data as Map<String, dynamic>)['message']?.toString() ?? 'Server error';
//       } else {
...
//   /// - حالات أخرى: يحاول update status => cancelled (والباك يرجّع خطأ لو ممنوع)
...
//         await _updateStatus(token: token, bookingId: bookingId, status: 'cancelled');
//         return true;
//       }
...
//   /// يحاول أكثر من route شائع لـ updateBookingStatus لتفادي اختلاف الراوت عندكم
